...
TMP_FILE=$(mktemp)
wget -q -O "$TMP_FILE" https://fw.koolcenter.com/iStoreOS/r5s/

# 使用 grep 提取所有 img.gz 文件的链接
LATEST_URL=$(grep -o 'istoreos-[0-9.]*-[0-9]*-r5s-squashfs.img.gz' "$TMP_FILE" | head -n 1)
...
# FULL_URL="https://fw.koolcenter.com/iStoreOS/r5s/$LATEST_URL"
FULL_URL="https://fw.koolcenter.com/iStoreOS/r5s/$LATEST_URL"
...
if [ -f "$README_FILE" ]; then
    ...
    CURRENT_URL=$(grep -o 'https://fw.koolcenter.com/iStoreOS/r5s/istoreos-[0-9.]*-[0-9]*-r5s-squashfs.img.gz' "$README_FILE" | head -n 1)
